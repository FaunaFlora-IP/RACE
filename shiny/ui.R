...
tabItem(tabName = "Flo_dat_car",
        ...
        fluidRow(
          bs4Card(
            ...
            fluidRow(
              column(
                ...
                div(
                  ...
                  tags$img(
                    src = "pv_ilustration.jpg",
                    alt = "Illustration of the nested sub-plot layout along a transect: sub-plot size, DBH range, category and class for Class C (10x10m, 5-15cm, Pancang), Class B (20x20m, 15-30cm, Tiang) and Class A (20x125m, >30cm, Pohon)",
                    style = "max-width: 100%; height: auto; border: 1px solid #dee2e6; border-radius: 4px;"
                  )
                ),
                radioButtons(
                  inputId = "class_size_source",
                  label = "Use the default design above, or a custom one for this survey?",
                  choices = c(
                    "Use the default design shown above" = "default",
                    "Use custom class definitions for this survey" = "custom"
                  ),
                  selected = "default"
                ),
                conditionalPanel(
                  condition = "input.class_size_source == 'custom'",
                  p("Enter the DBH range (shown as a label only, e.g. in the Mean Tree Density table - trees are already classified as A/B/C in your uploaded data, this doesn't reclassify them) and the actual sub-plot area in hectares for each class in this survey. Enter the area directly in hectares, e.g. ", tags$code("0.01"), " for a 10x10 m sub-plot or ", tags$code("5"), " for a 5-hectare sub-plot."),
                  fluidRow(
                    column(
                      width = 4,
                      strong("Class C"),
                      textInput("class_c_dbh", "DBH range (cm, for reference)", value = "5-15"),
                      numericInput("class_c_area_ha", "Sub-plot area (ha)", value = 0.01, min = 0.0001)
                    ),
                    column(
                      width = 4,
                      strong("Class B"),
                      textInput("class_b_dbh", "DBH range (cm, for reference)", value = "15-30"),
                      numericInput("class_b_area_ha", "Sub-plot area (ha)", value = 0.04, min = 0.0001)
                    ),
                    column(
                      width = 4,
                      strong("Class A"),
                      textInput("class_a_dbh", "DBH range (cm, for reference)", value = "> 30"),
                      numericInput("class_a_area_ha", "Sub-plot area (ha)", value = 0.25, min = 0.0001)
                    )
                  )
                )
              )
            ...
            tags$hr(),
            actionButton(inputId = "carbonconfirm", label = "Confirm Stratum, Wood Density & Plot Design")
          )
        ),
