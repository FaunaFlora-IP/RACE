...
class_size_factors <- reactive({
  if (identical(input$class_size_source, "custom")) {
    ...
    area_a <- input$class_a_area_ha
    area_b <- input$class_b_area_ha
    area_c <- input$class_c_area_ha
  } else {
    area_a <- 0.25  # Class A (Pohon) default sub-plot: 20x125 m = 0.25 ha
    area_b <- 0.04  # Class B (Tiang) default sub-plot: 20x20 m  = 0.04 ha
    area_c <- 0.01  # Class C (Pancang) default sub-plot: 10x10 m = 0.01 ha
  }
  c(A = 1 / area_a, B = 1 / area_b, C = 1 / area_c)
})

## DBH-range labels per class, for display only (not used in calculation -
## trees are already classified into A/B/C in the uploaded data; this just
## controls how each class is labeled in the Mean Tree Density table so it
## reflects whatever DBH ranges this survey actually used)----
class_dbh_labels <- reactive({
  if (identical(input$class_size_source, "custom")) {
    c(
      A = paste0("Class A (", input$class_a_dbh, " cm)"),
      B = paste0("Class B (", input$class_b_dbh, " cm)"),
      C = paste0("Class C (", input$class_c_dbh, " cm)")
    )
  } else {
    c(
      A = "Class A (> 30 cm)",
...
flora_confirmed <- eventReactive(input$carbonconfirm, {
  ...
  agb <- calc_AGB(wd_data, method = input$final_allometric_method,
                  dbh_col = "DBH", h_col = "TT", rho_col = "wood_density") %>%
    add_agb_tpha(factors = class_size_factors())
